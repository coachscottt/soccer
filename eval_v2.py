"""v2 judgment: xG-blended independent engine vs closing lines.

Alpha sweep (0 = v1 goals-only baseline; 0.5, 0.7 = xG blends), each
walk-forward by month on big-3 2025/26, judged on BOTH markets:
  1X2  vs devigged closing (loader CSVs)
  O/U 2.5 vs devigged closing totals (raw cache CSVs)

Run: python eval_v2.py
"""
import sqlite3
...
# --- preload league data once ---
data = {}
for lid in LEAGUES:
    matches = load_league_xg(conn, lid)
    xi = xi_quality(conn, lid)
    data[lid] = (matches, xi,
                 xi.set_index(["fixture_id", "team_id"])["xi_rating"])
    cov = matches[matches["Date"] < "2025-08-01"]["xg_home"].notna().mean()
    print(f"{LEAGUES[lid][0]}: xG coverage in training data {cov*100:.0f}%")

for alpha in ALPHAS:
    rows = []
    for lid, (lname, code) in LEAGUES.items():
        matches, xi, xi_lookup = data[lid]
        ...
        for month in sorted(season["Date"].dt.to_period("M").unique()):
            ...
            if len(train) < 500:
                continue
            eng = IndependentEngine(xg_alpha=alpha).fit(
    ...
    print(f"  O/U2.5 (n={len(m2)}): model {b(mo2):.4f}"
          f" vs close {b(mk2):.4f}  gap {b(mo2) - b(mk2):+.4f}"
          f"  (base-rate {b(np.full_like(mo2, y2.mean())):.4f})")
